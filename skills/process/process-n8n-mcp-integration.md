...
- **Write**: ONLY to `BACKLOG.md`.

### n8n Workflows (Slow/Write)
- **Role**: Lifecycle Automation (Drafting, Archiving).
- **Write**: Creates files in `drafts/`, updates Status.

## 2. Integration Flow
1.  **Suggest**: Agent calls `propose_skill` -> Entry in `BACKLOG`.
2.  **Watch**: n8n detects `pending` entry.
3.  **Draft**: n8n (via LLM) writes Draft to `drafts/`.
4.  **Notify**: Human gets alert to review.

## 3. Hard Constraints
- **Human Gate**: No auto-publish. Review required.
- **SSOT**: `BACKLOG.md` is the source of truth for pending work.
